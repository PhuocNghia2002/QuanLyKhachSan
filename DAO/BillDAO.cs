...
namespace QuanLyKhachSan.DAO
{
    public class BillDAO //Lấy ra billInfo
    {
        ...
        public bool UpdateBill(int idCustomer, int idRoom, int idRoomtype)
        {
            string query = string.Format("UPDATE Bill SET idCustomer = {0} , idRoom = {1} WHERE ID = {2}", idCustomer, idRoom, idRoomtype);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
        public bool DeleteBill(int idRoom)
        {
            string query = string.Format("DELETE FROM Bill WHERE idRoom = {0} ", idRoom);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
    }
