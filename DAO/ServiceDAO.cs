...
namespace QuanLyKhachSan.DAO
{
    public class ServiceDAO
    {
        ...
        public bool UpdateService(string nameService, float priceService, int id)
        {
            string query = string.Format("UPDATE Service SET NameService = N'{0}' , Price = {1} WHERE ID = {2}", nameService, priceService, id);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
        public bool DeleteService(int id)
        {
            string query = string.Format("DELETE FROM Service WHERE ID = {0}", id);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
    }
